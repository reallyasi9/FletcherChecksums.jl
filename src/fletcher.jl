...
struct Fletcher{T<:Unsigned} <: Checksum{T}
    c0::UInt64
    c1::UInt64
    modulo::UInt64
    blocksize::Int
...
function value(cs::Fletcher{T}) where {T}
    return ((cs.c0 & (typemax(T) >> (sizeof(T)*4))) | (cs.c1 << (sizeof(T)*4))) % T
end

function reset(cs::Fletcher{T}) where {T}
    return Flectcher{T}(0, 0, cs.modulo, cs.blocksize)
end

function update(cs::Fletcher{T}, data::AbstractVector{UInt8}) where {T}
    len = length(data)
    len == 0 && return cs
    ...
    return Fletcher{T}(c0, c1, cs.modulo, cs.blocksize)
end

function update(cs::Fletcher{T}, value::UInt8) where {T}
    c0 = cs.c0 + value
    if c0 > cs.modulo
        c0 -= cs.modulo
    end
...
function Fletcher16(init::UInt16 = zero(UInt16))
    c1 = init >> 8
    c0 = init & 0xff
    return Fletcher{UInt16}(c0, c1, UInt64(0xff), 380368696)
end

function Fletcher32(init::UInt32 = zero(UInt32))
    c1 = init >> 16
    c0 = init & 0xffff
    return Fletcher{UInt32}(c0, c1, UInt64(0xffff), 23726746)
end

function Fletcher64(init::UInt64 = zero(UInt64))
    c1 = init >> 32
    c0 = init & 0xffffffff
    return Fletcher{UInt64}(c0, c1, UInt64(0xffffffff), 92681)
end

function Fletcher16a(init::UInt16 = zero(UInt16))
    c1 = init >> 8
    c0 = init & 0xff
    return Fletcher{UInt16}(c0, c1, UInt64(0x100), 379625061)
end

function Fletcher32a(init::UInt32 = zero(UInt32))
    c1 = init >> 16
    c0 = init & 0xffff
    return Fletcher{UInt32}(c0, c1, UInt64(0x10000), 23726565)
end

function Fletcher64a(init::UInt64 = zero(UInt64))
    c1 = init >> 32
    c0 = init & 0xffffffff
    return Fletcher{UInt64}(c0, c1, UInt64(0x100000000), 92681)
end
...
function Adler32(init::UInt32 = one(UInt32))
    c1 = init >> 16
    c0 = init & 0xffff
    return Fletcher{UInt32}(c0, c1, UInt64(65521), 5552)
end
